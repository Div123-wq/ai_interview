...
def save_question_answer(session_id, q_num, question, q_type, answer, feedback, score, strengths, improvements, follow_up, time_taken):
    conn = get_db()
    conn.execute(
        """INSERT INTO session_questions
           (session_id, question_number, question, question_type, user_answer, ai_feedback, score, strengths, improvements, follow_up, time_taken)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (session_id, q_num, question, q_type, answer, feedback, score, strengths, improvements, follow_up, time_taken)
    )
    conn.commit()
    conn.close()
...
def finish_session(session_id, total_score, avg_score, completed):
    ...
    conn.execute(
        ...
        (total_score, avg_score, completed, datetime.utcnow(), session_id)
    )
    conn.commit()
    conn.close()
